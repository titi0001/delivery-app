...
import { requestFindSeller, requestUser } from '../services';
  ...
  useEffect(() => {
    const getOrders = async () => {
      ...
      if (findSeller) {
        ...
        const ordersBySeller = await requestFindSeller(findSeller.id);
        ...
        {orders?.map((e) => (
          <div key={ e.id }>
            ...
            >
              ...
              <p data-testid={ `seller_orders__element-delivery-status-${e.id}` }>
                {e.status}
              </p>
              <p data-testid={ `seller_orders__element-order-date-${e.id}` }>
                {`${String(new Date(e.saleDate).getUTCDate()).padStart(
                  2,
                  '0',
                )}/${String(new Date(e.saleDate).getUTCMonth() + 1).padStart(
                  2,
                  '0',
                )}/${new Date(e.saleDate).getFullYear()}`}
              </p>
